...
def sanitize(string, replacement_symbol="_"):
    ...
    string = re.subn(
        f"{re.escape(replacement_symbol)}" + r"{1,}", replacement_symbol, string
    )[0]
...
def transcode(
    ...
    if split:
        ...
        for chapter in metadata_chapters:
            ...
            logging.info(f"Obtained chapter metadata for chapter " f"{chapter_num}.")
            logging.debug(
                f"Chapter metadata is:\n"
        ...
        if remove_full:
            logging.info(f"Done splitting, removing decoded " f"{extension} file...")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        nargs="*",
        type=str,
        help="Directory/ies from which .aax files will be " "converted.",
    )
    ...
    parser.add_argument(
        "-b", "--debug", action="store_true", help="Set logging level to debug."
    )
    ...
    if not re.match(r"[0-9a-fA-F]+", activation_bytes):
        print("No valid authcode provided and no valid '.authcode' file " "found.")
        sys.exit(0)
